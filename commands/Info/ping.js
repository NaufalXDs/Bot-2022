    ...
    try {
      ...
      await message.reply({
        ...
        .then(newMsg => newMsg.edit({
          content: `:green_circle:  Bot Ping: \`${Math.floor((Date.now() - message.createdTimestamp) - 2 * Math.floor(client.ws.ping))} ms\`\n\n:green_circle: Api Ping: \`${Math.floor(client.ws.ping)} ms\``,
          ephemeral: true
        }).catch(e => {
